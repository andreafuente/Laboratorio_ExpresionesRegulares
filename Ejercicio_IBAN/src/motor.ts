...
const patron = /^ES(?<controlIban>\d{2})(\s|-)?(?<codbanco>\d{4})(\s|-)?(?<sucursal>\d{4})(\s|-)?(?<control>\d{2})(\s|-)?(?<cuenta>\d{10})$/;
...
export const IBANBienFormado = (valor: string): boolean => {
return patron.test(valor);
}
export const quitarEspaciosIBAN = (valor: string): string => {
...
export const IBANValido = (valor: string): boolean => {
  ...
  const IBANSinEspacios = quitarEspaciosIBAN(valor);
  return ibantools.isValidIBAN(IBANSinEspacios);
};

export const obtenerDatosIBAN = (valor: string): { nombreBanco: string; sucursal: string; control: string; cuenta: string } | null => {
    const resultado = patron.exec(valor);
    if (!resultado || !resultado.groups) {
        return null;
    }
    const { codbanco, sucursal, control, cuenta } = resultado.groups;
    return {
    nombreBanco: bancos[codbanco] ?? "Banco desconocido",
    sucursal,
    control,
    cuenta,
  };
